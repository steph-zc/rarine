...
import { extractHexFromName, nameWithHex, stripHexFromName } from '../utils/corBordado'
...
// Luminosidade relativa — decide texto preto ou branco em cima da cor
...
function Cores() {
  ...
  // Hex embutido no nome: "Azul Royal (#3B82F6)"
  const formInicial = { name: '', threadCode: '', hexColor: '#3b82f6' }
  const [form, setForm] = useState(formInicial)

  // Estado dos sliders RGB separados para o picker
  const [rgb, setRgb] = useState({ r: 59, g: 130, b: 246 })

  // Sincroniza slider → hex
  ...
  // Sincroniza hex (color input nativo) → sliders
  ...
  const abrirEditar = (cor) => {
    const hex = extractHexFromName(cor.name) || cor.hexColor || '#3b82f6'
    setForm({
      name: stripHexFromName(cor.name),
      threadCode: cor.threadCode,
      hexColor: hex,
    })
    setRgb(hexToRgb(hex))
    setEditando(cor.id)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        name: nameWithHex(form.name, form.hexColor),
        threadCode: form.threadCode,
      }
  ...
  return (
    <div>
      <Header />
      <div className="page-title-bar">Cores de Bordado</div>
      ...
      <div className="page-content">
        ...
        {mostrarForm && (
          <div className="form-card" style={{ maxWidth: 480 }}>
            <h6>{editando ? 'Editar Cor' : 'Nova Cor de Bordado'}</h6>
            <form onSubmit={handleSubmit}>

              {/* Preview da cor */}
              <div style={{
                background: form.hexColor,
                borderRadius: 10,
                height: 64,
                marginBottom: 16,
                display: 'flex',
                ...
                <span style={{
                  fontWeight: 700,
                  fontSize: 15,
                  color: isLight(form.hexColor) ? '#1a2233' : '#ffffff',
                  letterSpacing: 0.5,
                }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <input
                    ...
                    style={{
                      width: 40, height: 36, padding: 2, border: '1.5px solid var(--cinza-borda)',
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button type="button" className="btn-secondary-r" onClick={() => setMostrarForm(false)}>
                  Cancelar
                </button>
              </div>
            </form>
        ...
        {!loading && !erro && (
          <div className="r-table-wrap">
            <table className="r-table">
              ...
              <tbody>
                {cores.map(c => {
                  const hex = extractHexFromName(c.name) || c.hexColor || '#cbd5e1'
                  ...
                  return (
                    <tr key={c.id}>
                      ...
                      <td>
                        <button className="btn-tabela btn-editar" onClick={() => abrirEditar(c)}>
                          Editar
                        </button>
                      </td>
                    </tr>
                ...
                {cores.length === 0 && (
                  <tr>
                    <td colSpan={5} style={{ textAlign: 'center', color: '#9ca3af', padding: '32px 0', fontSize: 14 }}>
