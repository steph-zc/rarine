...
// Valores exatos definidos no banco — exibidos com inicial maiúscula
const MODELOS = [
...
function Produtos() {
  ...
  const formInicial = { name: '', type: 'camiseta', model: '', collar: '', fabric: '', baseColor: '', hasEmbroideryOrPrint: false }
  ...
  const abrirNovo = () => { setForm(formInicial); setEditando(null); setMostrarForm(true) }
  const abrirEditar = (p) => {
    setForm({
      name: p.name || '',
  ...
  return (
    <div>
      <Header />
      <div className="page-title-bar">Produtos</div>
      ...
      <div className="page-content">
        ...
        {mostrarForm && (
          <div className="form-card" style={{ maxWidth: 580 }}>
            <h6>{editando ? 'Editar Produto' : 'Novo Produto'}</h6>
                  ...
                  <tr key={p.id}>
                    ...
                    <td>
                      {(p.hasEmbroidery || p.hasPrint)
                        ? <span style={{ color: '#065f46', fontWeight: 600 }}>Sim</span>
